...
# Ayuda: Comando seq que se usar· en un for para ir seleccionando 
...
for (j in seq(corpus))
{
  ...
  corpus[[j]] <- gsub("corrupci<>n", "corrupci√≥n", corpus[[j]])
  corpus[[j]] <- gsub("instituci<>n", "instituci√≥n", corpus[[j]])
}
...
# Esto deber√≠a funcionar. Probar. ¬øEn Mac, no?

# tweets <- tm_map(tweets, removeWords, c("<d5>", "<d1>", "\f", "?", "<d1>"))
# LÌnea anterior comentada porque estos cÛdigos no 
...
# Solo elimina la  primera cadena
